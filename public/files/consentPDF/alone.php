...
if (!empty($arrayPost)) {
    ...
    if (!empty($credentials['CLECID'])) {
        ...
        // $db = connections();
        // $stmt = $db->prepare('SELECT * FROM ' . $configs['table'] . ' WHERE order_id=?');
        // $stmt->execute([$orderId]);
        // $row = $stmt->fetch(PDO::FETCH_ASSOC);
        //if (!empty($row)) {
            $first_name = $arrayPost['firstName'];
            $second_name = $arrayPost['lastName'];
            $date_create = date('mdY');
            $docName = $orderId .'_'. $second_name . "_" . rand(0, 9999) . ".pdf";
            $signature_text = $first_name." ".$second_name;
            ...
            $address = $arrayPost['address1'];
            $address2 = $arrayPost['address2'];
            $city = $arrayPost['city'];
            $state = $arrayPost['state'];
            $zipcode = $arrayPost['zipcode'];
            $email = $arrayPost['email'];
            $phone_number = $arrayPost['phone'];
            // $dob = $row['dob'];
            $ssn = $arrayPost['ssn'];
            // $tribal_id = $row['tribal_id'];
            // $dobParts = explode('/', $dob);
            $month = $arrayPost["dmonth"];
            $day = $arrayPost["dday"];
            $year = $arrayPost["dyear"];
            // /**************************/
            $shipping_address1 = $arrayPost['baddress1'];
            $shipping_address2 = $arrayPost['baddress2'];
            $shipping_city = $arrayPost['bcity'];
            $shipping_state = $arrayPost['bstate'];
            $shipping_zipcode = $arrayPost['bzipcode'];
            // /**************************/
            ...
            $pdfPath = '../../../../../' . $configs['consent'] . $docName;
